...
def get_data_oplr(func_space, x_range, t_range, n_funcs, n_x, n_t, n_res, n_sensor, device='cpu'):
    """
    ...
    Returns:
        ...
        sensor_x: float32 tensor of shape (n_sensor, 1) — sensor positions for branch input evaluation
    """
    ...
    # Sensor positions — float32 tensors, consistent with all other x/t coordinates
    ...
    return funcs, x, t, data_params, sensor_x
...
def get_func_rep(func_space, funcs, sensor_x, device='cpu'):
    ...
    Args:
        ...
        funcs: NumPy array of shape (n_funcs, n_features)
        sensor_x: float32 tensor of shape (n_sensor, 1) — sensor positions (endpoint=False)
    ...
    return torch.tensor(branch, dtype=torch.float32).to(device)
...
def get_pde_oplr(pde_name, pde_params_list, loss_name, funcspace_params_list=None):
    """
    ...
    if pde_name == "convection":
        ...
        func_space = dde.data.GRF(
            T=np.float32(x_range[1]),   # Use float32 so interp range matches float32 tensor precision
        ...
        def loss_func(func_space, funcs, x, t, pred):
            ...
            # Keep as float32: GRF T is also float32, so interp range max matches query pts max
            ic_targets_np = func_space.eval_batch(funcs, x_left.cpu().detach().numpy())  # (n_funcs, n_x)
            ic_targets = torch.tensor(ic_targets_np, dtype=torch.float32).to(x_res.device)
    ...
    return func_space, x_range, t_range, loss_func, pde_coefs
...
def get_model_oplr(model_type, n_sensors, branch_hidden, trunk_hidden, activation="sin", kernel_init="Glorot normal"):
    ...
    Args:
        ...
        n_sensors (int): Number of branch input sensors (= n_x)
...
def train_oplr(model,
    ...
    """
    func_space, x_range, t_range, loss_func, pde_coefs = get_pde_oplr(pde_name, pde_params, loss_name, funcspace_params_list)
    ...
    funcs, x, t, data_params, sensor_x = get_data_oplr(func_space, x_range, t_range, n_funcs, n_x, n_t, n_res, n_sensor, device)
    branch_inputs = get_func_rep(func_space, funcs, sensor_x, device)  # (n_funcs, n_sensors)

    # Initial evaluation before training
    # Note: cannot use torch.no_grad() here because loss_func computes PDE residuals via autograd
    model.eval()
    ...
    funcs_test, x_test, t_test, data_params_test, _ = get_data_oplr(
        func_space, x_range, t_range, n_funcs_test, n_x_test, n_t_test, n_res, n_sensor, device)
